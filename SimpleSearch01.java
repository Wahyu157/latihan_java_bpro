public class SimpleSearch01{
  ...
  public static int cariByNim(daftarNim )
	{
		int idx=-1;
		int i=0;
		do{
  ...
  public static void main(String []str){
	String nim="13410100157"
	int hasilCari = cariIdxNim(nim);
	if(hasilCari==-1)
		System.out.println("Nim: " +nim +
  }
  public static String []filterNama(String nama) {
  //hitung jumlah data yg mengandung kata nama
  int counter=0;
  for(int i=0;i<daftarNama.length;i++){
	if(daftarNama[i].countain(nama))
		counter++;
  }
  //buat array of string dg panjang jumlah pd lngkah 1
  String []arr=new String(counter);
  //lakukan looping lagi untuk mengecek nama
  counter=0;
  for (int i=0;i<daftarNama.length;i++) {
  //jika mengandung nama yg dicari maka isikan array of string 2
